...
```
(venv) path_to_django_rest_auth_email_confirm_reset\project_sample\backend_django: tox
``` 
...
```
path_to_django_rest_auth_email_confirm_reset: test_after_packaging_with_tox.py
```
It is not exclude choosing the correct work stage in the file **.env**:  
```python
WORK_STAGE = "test_after_packaging_with_tox" 
